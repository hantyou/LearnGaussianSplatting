...
def main():
    ...
    cameras=[street_camera(a,args.size) for a in train_angles+heldout_angles]
    with torch.no_grad():
    ...
    np.savez(out/"learned_scene.npz",**{k:v.detach().numpy() for k,v in student.state_dict().items()})
    save_street_preview(teacher,load_scene_json(data/"street-before.json"),student,out/"comparison.png",core)
    report={"gaussians":len(student.means),"steps":args.steps,"size":args.size,
...
def save_street_preview(teacher,before,after,path,core=None):
    if core is None:
        from . import core
    cameras=[street_camera(a,96) for a in [0,30]]
    targets=[render(teacher,c)[0] for c in cameras]
